...
def _build_judge():
    ...
    def _is_finished(response):
        for g in response.flatten():
            resp = g.generations[0][0]
            if resp.generation_info is not None:
                reason = resp.generation_info.get("finish_reason")
                if reason is not None and reason not in ["stop", "STOP", "length", "MAX_TOKENS", "eos_token"]:
                    return False
    ...
    llm = LangchainLLMWrapper(chat_llm, is_finished_parser=_is_finished)
...
def _score_df(metric_key: str, samples: list, scores) -> pd.DataFrame:
    return pd.DataFrame([
        {"question": s["question"][:65], metric_key: round(float(getattr(r, "value", r)), 3)}
...
async def _batched_score(metric, inputs: list, samples: list, status_cb=None, label: str = "") -> list:
    """
    Runs single_turn_ascore in chunks of GENERAL_BATCH_SIZE with cooldowns between chunks.
    ...
    for b_idx, batch in enumerate(batches):
        ...
        batch_tasks = [metric.single_turn_ascore(sample) for sample in batch]
        scores = await asyncio.gather(*batch_tasks)
