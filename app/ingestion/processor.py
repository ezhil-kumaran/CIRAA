...
def process_file(file_path: str, filename: str, source_type: str):
    ...
    with logfire.span("Processing File", file=filename, source=source_type):
        try:
            ...
            # 4. Embed and index in Qdrant in batches of 50
            with logfire.span("Vectorizing & Indexing"):
                ...
                # Batch upsert to prevent HTTP write timeouts on large PDFs
        ...
        if wipe:
            with logfire.span("Wiping Collection"):
                if qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
                    ...
                    logfire.info(f"Collection '{settings.QDRANT_COLLECTION}' deletion requested.")
                    for _ in range(10):
                        time.sleep(1)
        ...
        if not qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
            ...
            try:
                ...
                if "already exists" in str(e).lower() or "409" in str(e):
                    logfire.info(f"Collection '{settings.QDRANT_COLLECTION}' already exists.")
                else:
                    raise
